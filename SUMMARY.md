# Indice

* [Introduzione](README.md)
* [GUI: Navigazione](navigation.md)
* [GUI: Progetti](projects.md)
* [CLI: Riga di comando](CLI.md)
* [API: Python SDK](api-python-sdk.md)
* [Fotocamere supportate](supported-cameras.md)
* [Formati immagine di output](output-image-formats.md)
* [Chloros+ Login](chloros+-login.md)
* [Obiettivi di calibrazione](calibration-targets.md)
* [Lingue supportate](supported-languages.md)
* [Download](download.md)
* [Domande frequenti](faq.md)

## Elaborazione delle immagini (GUI)

* [Regolazione delle impostazioni del progetto](processing-images-gui/adjusting-project-settings.md)
* [Aggiunta di file a un progetto](processing-images-gui/adding-files-to-a-project.md)
* [Selezione delle immagini di destinazione](processing-images-gui/choosing-target-images.md)
* [Avvio dell&#x27;elaborazione](processing-images-gui/starting-the-processing.md)
* [Monitoraggio dell&#x27;elaborazione](processing-images-gui/monitoring-the-processing.md)
* [Completamento dell&#x27;elaborazione](processing-images-gui/finishing-the-processing.md)

## IMPOSTAZIONI DEL PROGETTO

* [Impostazioni del progetto](project-settings/project-settings.md)
* [Formule dell&#x27;indice multispettrale](project-settings/multispectral-index-formulas.md)

## VISUALIZZATORE DI IMMAGINI (GUI)

* [Griglia delle immagini](image-viewer-gui/image-grid.md)
* [Apertura di un&#x27;immagine a schermo intero](image-viewer-gui/opening-an-image-full-screen.md)
* [Livelli immagine](image-viewer-gui/image-layers.md)
* [Sandbox indice/LUT](image-viewer-gui/index-lut-sandbox.md)
* [Indicatori mappa](image-viewer-gui/map-markers.md)
